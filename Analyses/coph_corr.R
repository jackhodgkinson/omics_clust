## Testing cophenetic correlation coefficient for determining number of views
# Load packages
library(mclust)
library(NbClust)
library(tidyverse)
...
cc_results <- data.frame(n_groups = integer(), ccc = numeric())
n_groups <- c(1:10)
for (i in n_groups) {
  ...
  cc_results <- rbind(cc_results, data.frame(n_groups = i, ccc = coph_cor))
}

# Plot the results
ggplot(cc_results, aes(x = n_groups, y = ccc)) +
  geom_line() +
  geom_point() + 
  labs(x = "Number of Groups",
       y = "Cophenetic Correlation Coefficient")

# Tested 1-5 groups and 1-10 groups with 10 cols, 1-50 groups with 100 cols and 1-5000 groups with 10000 cols, always lowest 
# coph corr for 1 group. Could be something in this! 
# Could do up to ceiling(log(ncol()))?
# The rest are always nearly 1!