...
# Set working directory for R functions 
setwd("~/Documents/MPhil Population Health Sciences/Dissertation/omics_clust")

# Load external functions
source("~/R Functions/simulateGMM.R")
source("~/R Functions/GMMclassifier.R")
...
models <- c("EII","VII","EEI","VEI","EVI","VVI","EEE","VEE","EVE","VVE",
            "EEV","VEV","EVV","VVV")
for (m in models) {
  start_time <- Sys.time()
  ...
  test2 <- mclust::Mclust(mds, modelName = m, G = g)
...
#write.csv(results, file = "~/Results/mds_threshold_singleTimepoint_results.csv", row.names = FALSE)